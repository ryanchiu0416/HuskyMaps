...
public class LinkedIntList {
    ...
    public static LinkedIntList concatenated(LinkedIntList A, LinkedIntList B) {
        ...
        if (currA != null) {
            result.front = new ListNode(currA.data);
            currA = currA.next;
            ListNode tmp = result.front;
            while (currA != null || currB != null) {
                if (currA != null) {
                    tmp.next = new ListNode(currA.data);
                    currA = currA.next;
                } else {
                    tmp.next = new ListNode(currB.data);
                    currB = currB.next;
                }
                tmp = tmp.next;
            }
        } else if (currB != null) {
            result.front = new ListNode(currB.data);
            currB = currB.next;
            ListNode tmp = result.front;
            while (currB != null) {
                tmp.next = new ListNode(currB.data);
